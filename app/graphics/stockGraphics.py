...
class StockGraphics:
    ...
    def create_histogram(self, pandas_df: DataFrame, path: str) -> NoReturn:
    ...
    def create_boxplot(self, pandas_df: DataFrame, path: str) -> NoReturn:
        self.__single_template("Plotting Stocks", pandas_df, pandas_df)
        plt.boxplot(pandas_df)
        plt.savefig(path)

    def create_multi_boxplot(self, pandas_df: DataFrame, path: str) -> NoReturn:
    ...
    def create_scatter(self, pandas_x_df: DataFrame, pandas_y_df: DataFrame, path: str) -> NoReturn:
        self.__single_template("Stock compare", pandas_x_df, pandas_y_df)
        plt.scatter(pandas_x_df, pandas_y_df)
        plt.savefig(path)

    def create_plot(self, pandas_x_df: DataFrame, pandas_y_df: DataFrame, path: str) -> NoReturn:
    ...
    def create_multi_plot(self, pandas_x_df: DataFrame, pandas_y_df: DataFrame, path: str) -> NoReturn:
